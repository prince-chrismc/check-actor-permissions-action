...
A GitHub Action to check if the current
[actor](https://docs.github.com/en/graphql/reference/interfaces#actor) has
sufficient access to the repository.

Actors may have one of
[four permission levels](https://docs.github.com/en/rest/collaborators/collaborators#get-repository-permissions-for-a-user)
for any repository:
...
This action will check the current repository to see if the actor has a high
enough permission level based on the provided argument.

## Getting Started

To have an action that only passes if the actor has write access to the repo,
use the action as follows:

```yaml
steps:
  - name: Enforce permission requirement
    uses: prince-chrismc/check-actor-permissions-action@v4
    with:
      permission: write
```
...
To have the workflow change behaviors depending on the actor's permissions, use
